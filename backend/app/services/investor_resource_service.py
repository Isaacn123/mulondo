...
from app.schemas.investor_resource import InvestorResource, InvestorResourceList

BASE_DIR = Path(__file__).resolve().parent.parent
RESOURCES_FILE = BASE_DIR / "data" / "investor_resources.json"


def _row_to_resource(row: InvestorResourceRow) -> InvestorResource:
...
def _load_from_json() -> InvestorResourceList:
    if not RESOURCES_FILE.exists():
        return InvestorResourceList()
    data = json.loads(RESOURCES_FILE.read_text(encoding="utf-8"))
    return InvestorResourceList.model_validate(data)


def _save_to_json(content: InvestorResourceList) -> None:
...
def _load_from_db(db: Session) -> InvestorResourceList:
    rows = db.query(InvestorResourceRow).order_by(
        InvestorResourceRow.sort_order.desc(),
        InvestorResourceRow.title,
    )
    return InvestorResourceList(items=[_row_to_resource(row) for row in rows])


def list_resources(*, published_only: bool = False) -> list[InvestorResource]:
...
def get_resource(slug: str) -> InvestorResource | None:
...
def _save_resource_to_db(db: Session, resource: InvestorResource) -> InvestorResource:
    row = db.query(InvestorResourceRow).filter(InvestorResourceRow.slug == resource.slug).one_or_none()
    if row is None:
        row = InvestorResourceRow(
            slug=resource.slug,
...
def _save_resource_to_json(resource: InvestorResource) -> InvestorResource:
...
def save_resource(resource: InvestorResource) -> InvestorResource:
    if get_settings().storage_backend == "database":
        db = SessionLocal()
        try:
...
def update_resource(slug: str, **fields) -> InvestorResource | None:
...
def delete_resource(slug: str) -> bool:
    if get_settings().storage_backend == "database":
        db = SessionLocal()
        try:
            row = db.query(InvestorResourceRow).filter(InvestorResourceRow.slug == slug).one_or_none()
            if row is None:
                return False
            db.delete(row)
            db.commit()
